...
## Prevent suffocation
execute as @s[tag=ts.sa.protected,scores={ts.sa.timer=-200..}] at @s anchored eyes if block ^ ^-0.1 ^ #spawnanimations:nonsolid if block ^ ^0.9 ^ #spawnanimations:nonsolid run function spawnanimations:internal/animation/dig_up/unprotect
